...
void main() {
  ...
  group('MentionTextEditingController - 核心功能', () {
    ...
    testWidgets('buildTextSpan', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(
          body: Builder(
            builder: (context) => Text.rich(c1.buildTextSpan(context: context, style: const TextStyle(), withComposing: false)),
          ),
        ),
      ));
      ...
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(
          body: Builder(
            builder: (context) => Text.rich(c2.buildTextSpan(context: context, style: const TextStyle(), withComposing: false)),
          ),
        ),
      ));
  ...
  group('MentionTextEditingController - 边界情况', () {
    ...
    testWidgets('光标调整', (WidgetTester tester) async {
      ...
      controller.value = TextEditingValue(text: driverText, selection: TextSelection.collapsed(offset: mentionPos - 1));
      controller.value = TextEditingValue(text: driverText, selection: TextSelection.collapsed(offset: mentionPos));
      await tester.pump();
  ...
  group('MentionTextField - Widget', () {
    ...
    testWidgets('复制为接近 displayText，粘贴回输入框可还原 mention', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(MaterialApp(home: Scaffold(body: MentionTextField(controller: dst))));
  ...
  group('MentionTextEditingController - 完整用户场景', () {
    ...
    testWidgets('场景2: 多个 mention 的完整流程', (WidgetTester tester) async {
      ...
      controller.value =
          TextEditingValue(text: '$driverText2@', selection: TextSelection.collapsed(offset: driverText2.length + 1));
      await tester.pump();
